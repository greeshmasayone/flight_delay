...
def get_flight_schema(spark, flights_raw):
    """Gets flights schema from the first retrived file in flights_raw

    Args:
        spark (SparkSession): spark session 
        flights_raw (FlightsRaw): FlightsRaw class availble in flights_raw.py

    Returns:
        schema: spak schema object
    """
    dataframe = load_csv_to_dataframe(spark, flights_raw.retrieved_files[0])
...
def create_or_update_flight_gold(spark, flight_silver_table, flight_gold_table, flight_gold_bath):
    """Create summary table aggregated by date
...
def create_or_update_date_table(spark, table, dim_date_table_name, dim_date_location_path):
    ...
    Args:
        ...
        dim_date_location_path (str): path to the location on disk to save the delta table
    Returns:
