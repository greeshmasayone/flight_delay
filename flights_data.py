...
class FlightsRaw():
    ...
    def retrieve(self, num_files=1, multiprocess=False):
        """Prepare the raw data by splitting the full year files into months
        ...
        # Check if number of files left less than what asked to retrieve
        if len(self.source) < num_files:
            num_files = len(self.source)
            print(f"Only {num_files} files left to retrieve")
    ...
    def _process_file(self, filepath, progress_bar):
        """Helper function to actually process file by file we call it from retrieve either by multiprocessing or normal
        ...
        Returns:
            tuple: tuple of (filepath, num_rows, retrieved_files) to update the class attributes
        """
        temp_folder = os.path.join(os.getcwd(), '.temp')
        ...
        # retrieved_files = []
        progress_bar.set_description(f"Retrieving... file: {filename}")
        for month in months:
            progress_bar.set_description(f"Retrieving... file: {filename} month: {str(month)}")
            # Filtering by month
            ...
            # retrieved_files.append(save_path)
            self.retrieved_files.append(save_path)
            progress_bar.update()
        self.retrieved[filename] = num_rows
        self.rows_retrieved += num_rows
        self.source.remove(filepath)
        progress_bar.set_description(f"Done... file: {filename}")
        logging.info(f'Retrieved file: {filename} with total number of rows: {num_rows:,}')
    ...
    def load(self):
        ...
        self.retrieved = state_dict['retrieved']
        self.rows_retrieved = state_dict['rows_retrieved']
        self.retrieved_files = state_dict['retrieved_files']
    ...
    def __str__(self):
        ...
        Target: {self.target}
        Rows Retrieved: {self.rows_retrieved:,}
        Number of Retrieved Files: {len(self.retrieved_files):,} files"""
